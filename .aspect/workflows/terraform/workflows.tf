...
module "remote_cache_dns" {
  ...
  zones = {
    "rules-deno-remote-cache.aspect.build" = {
      domain_name = "rules-deno-remote-cache.aspect.build"
      comment     = "rules-deno-remote-cache.aspect.build"
      tags = {
        Name = "rules-deno-remote-cache.aspect.build"
      }
    }
  }
}
...
module "aspect_workflows" {
  ...
  external_remote_cache = {
    public_hosted_zone_id = module.remote_cache_dns.route53_zone_zone_id["rules-deno-remote-cache.aspect.build"]
    cache_shards          = 1
    frontend = {
      max_scaling = 5
      min_scaling = 1
